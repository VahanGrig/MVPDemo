...
extension UIView {
    static func loadFromNib() -> Self {
            ...
            let loadedNibFirst = bundle.loadNibNamed(String(describing: Self.self),
                                                     owner: nil,
                                                     options: nil)?.first
            
            guard let view = loadedNibFirst as? Self else {
                preconditionFailure("View is not loaded or has wrong type")
            }
